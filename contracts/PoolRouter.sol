...
contract PoolRouter is OwnableTwoSteps, ReentrancyGuard, Pausable {
    ...
    function deposit(address token, uint256 amount) external payable nonReentrant whenNotPaused {
        ...
        IERC20(token).transferFrom(msg.sender, owner, depositFee);
    ...
    function _validateDepositAmount(address pool, uint256 amount) private view {
        ...
        if (amount > depositLimit[pool].maxDepositAmount ||
            IERC20(IPool(pool).asset()).balanceOf(pool) + amount + pendingDeposits[pool] > depositLimit[pool].maxBalance)
        {
