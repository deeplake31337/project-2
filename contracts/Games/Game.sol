...
abstract contract Game is ReentrancyGuard, VRFConsumerBaseV2, OwnableTwoSteps {
    struct GameParams {
        uint40 blockNumber;
    ...
    function _transferVrfFee(uint256 vrfFee) internal {
        if (vrfFee > 0) {
            GAME_MANAGER.vrfFeeAddress().call{value: vrfFee}();
        }
    }
    ...
    function _refund(GameParams storage params) internal {
        ...
        if (params.randomnessRequestedAt + GAME_MANAGER.timeForRefund() > block.timestamp) {
    ...
    function _checkVrfResponse(uint40 randomnessRequestedAt) internal view returns (bool executable) {
        executable = (randomnessRequestedAt - 5 minutes + GAME_MANAGER.timeForRefund() > block.timestamp);
    }
